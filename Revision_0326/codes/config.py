...
class ProblemConfig:
    ...
    def __init__(self, instance_type='full'):
        ...
        # Data file paths
        import os as _os
        self.data_file = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), '..', 'data', f"DH_data_{instance_type}.pkl")
...
class DataParameters:
    ...
    # Economic parameters
    S = 150.0  # Unit selling price (increased from 100 for positive profits)
    ...
    C_plant_min = 3500   # Reduced from 5000
    C_plant_max = 7000   # Reduced from 10000

    # DC fixed costs: C^dc_j - REDUCED for positive profits
    C_dc_min = 2000      # Reduced from 3000
    C_dc_max = 4000      # Reduced from 6000

    # Ordering costs at DCs: O_j
    O_min = 3500         # Reduced from 5000
    O_max = 10000        # Reduced from 15000
...
class SensitivityConfig:
    ...
    def __init__(self, R):
        ...
        self.output_file = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), '..', 'result', 'DH_sensitivity_results.csv')
        self.plot_file = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), '..', 'result', 'DH_sensitivity_plot.png')
