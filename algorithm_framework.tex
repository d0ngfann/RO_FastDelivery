...
\begin{itemize}
    \item $x_i \in \{0,1\}$: 1 if plant $i$ is opened, 0 otherwise.
    \item $y_j \in \{0,1\}$: 1 if DC $j$ is opened, 0 otherwise.
    \item $z_{ij} \in \{0,1\}$: 1 if DC $j$ is supplied by plant $i$, 0 otherwise.
...
\begin{align}
    ...
    &\quad + \sum_{k \in K} \sum_{i \in I} \sum_{j \in J} L_{kij}^1 z_{ij} + \sum_{j \in J} \sum_{r \in R} L_{jr}^2 w_{jr} \quad \text{(Fixed Setup Costs)}
\end{align}
...
\begin{align}
    ...
    & z_{ij} \le x_i, \quad \forall i \in I, j \in J \label{eq:mp_plant_open} \\
    & z_{ij} \le y_j, \quad \forall i \in I, j \in J \label{eq:mp_dc_open} \\
    ...
    & A_{ij}^{k(l)} \le MC_j z_{ij}, \quad \forall k \in K, i \in I, j \in J, l \in \mathcal{L} \label{eq:mp_route_ij} \\
...
\begin{align}
    \textbf{[SP-Dual]} \quad Z_{SP} = \min \quad & \sum_{k \in K} \sum_{i \in I} MP_{ki} \pi_{ki} + \sum_{j \in J} MC_j \sigma_j + \sum_{k \in K} \sum_{i \in I} \sum_{j \in J} MC_j \hat{z}_{ij} \psi_{kij} \notag \\
