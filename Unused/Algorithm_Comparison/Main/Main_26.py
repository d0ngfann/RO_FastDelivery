...
# ──────────────────────────────────────────────────────────────────────────────
from evaluate_HD_26 import evaluate
from constraint_utils_numpy_26 import load_s_table, adjust_chromosome
    ...
    for _ in range(3):
        ...
        if rand_choice < 0.25:
            ...
            if len(one_indices) >= 1:
                indices_to_flip = random.sample(list(one_indices), 1)
...
def memetic_algorithm(fitness_cache):
    ...
    with ProcessPoolExecutor(max_workers=os.cpu_count(), initializer=init_worker,
        ...
        # [NEW] Time tracking variable for forced execution every 20 minutes
        ...
        while time.time() - start_time < TIME_LIMIT_SECONDS:
            ...
            # --- Global stagnation or 20-minute periodic forced execution logic ---
            # Execute 'shock therapy' when 20 minutes have passed or global stagnation reaches 10 generations
            ...
            if time_trigger or stagnation_trigger:
                ...
                # 2. Create a random mapping to transform the d-vector
                d_map = {i: random.randint(1, 10) for i in range(1, 11)}

                shake_up_futs = []
                # 3. Change the d-vectors of the selected individuals and add them to the re-evaluation list
                for idx in indices_to_shake:
                    ind = population[idx]
                    d_start = N_BINARY
                    d_end = N_BINARY + N_CUSTOMER
                    # Change d-vector values using the map
                    ind['chrom'][d_start:d_end] = [d_map.get(val, val) for val in ind['chrom'][d_start:d_end]]
                    # Set fitness to None for re-evaluation
                    ind['fitness'] = None
                    shake_up_futs.append(pool.submit(adjust_and_evaluate_cached, ind))

                # 4. Re-evaluate the changed individuals
                if shake_up_futs:
                    for fut in tqdm(as_completed(shake_up_futs), total=len(shake_up_futs),
                                    desc="   - D-Vector Shake-up Eval", leave=False):
                        # The population list has already been changed, so just wait for the results
                        fut.result()

                # 5. Reset counters and time
                global_stagnant = 0
                last_d_vector_shake_up_time = elapsed_time  # IMPORTANT: Update time record

                # 6. Re-sort to reflect in elites and the next generation's composition
                population.sort(key=lambda x: x.get('fitness') or -math.inf, reverse=True)
                elites = [ind.copy() for ind in population[:num_elites]]

            # After 5400 seconds, the algorithm turns into Exploit mode
