"""
DH_data_gen.py
Data generation module for the robust supply chain optimization problem.
Generates synthetic data including costs, capacities, demands, and distances.
"""

import numpy as np
import pickle
import os
from DH_config import ProblemConfig, DataParameters
...
def generate_DI_vector(k, M=3):
    """
    Generate DI vector using the formula: y = exp((ln(3/2)) * k * x) = (3/2)^(k*x)
    ...
    """
    return [1.5 ** (k * m) for m in range(M)]
...
def generate_DI_scenarios(K=3, M=3, seed=None):
    ...
    Formula: DI_m = (3/2)^(k*m) for m = 0, 1, 2
...
def generate_supply_chain_data(config: ProblemConfig, seed=42):
    ...
    # ========== Capacities ==========
    # Plant capacities MP_{ki}
    for k in range(K):
        for i in range(I):
            data.MP[(k, i)] = np.random.uniform(params.MP_min, params.MP_max)

    # DC capacities MC_j
    for j in range(J):
        data.MC[j] = np.random.uniform(params.MC_min, params.MC_max)
